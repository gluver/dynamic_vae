...
class Train:
    ...
    def __init__(self, args):
        """
        Training module initialization, loading project parameters and creating model save path

        Args: 
            args：
                Default parameters,class Namespace,
            normalizer：
                Normalization method, class utils.Normalizer or None
            data：
                Training data, class dataset.SlidingWindowBattery or None
            label_data：
                Data mileage label, class tasks.Label or None
            data_task：
                The model of task  ，class tasks.Task or None
            **kwargs etc.
        """
        self.args= args
        ...
        # Create a model save path and add the associated path to the ARGS for subsequent calls  
    ...
    def main(self):
        """
        Training main program ,Load training data, preprocess, create model and train according to parameters, save model results and related parameters  
        train: normalized data used for model input.train[i][0]is an array with shape m * N.
                train[i][1]is a dict containing label, CAR, charge_segment, Mileage, and timestamp
        model： models used for training, including DynamicVAE  
        loss: nll kl label
        rec_error: Reconstruction error, calculated by calculating the MSE of log_p and target  
        
        """     
        print("Loading data to memory. This may take a few minutes...")
        data_pre = dataset.Dataset(self.args.train_path)
        self.normalizer = Normalizer(dfs=[data_pre[i][0] for i in range(20)], variable_length=self.args.variable_length)
        ...
        print("model", model)
        # A way to specify the optimizer and update the learning rate
        ...
        # Load the data with the DataLoader
    ...
    def model_result_save(self, model):
        """
        To save pretrain model ,normalization pkl and training parameters
        args: 
            model: training of vae model
        """
    ...
    def loss_fn(self, log_p, target, mean, log_v):
        """
        Calculate NLL_loss, KL_loss, and KL_weight of KL_loss.

        args: 
            log_p: training output of model
            target: training target selected by task
            mean: mean of vae model
            log_v: variance of training characteristics

        return:
            nll_loss: Negative Log Likelihood loss
            kl_loss: KL Divergence
            kl_weight: float, anneal0, changed by anneal_function
    ...
    def loss_visual(self):
        """
        Visualization of each loss in the training process
        """
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', type=str,
                        default=os.path.join( 'params.json'))
